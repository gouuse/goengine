...
import com.gouuse.goengine.http.interceptor.GzipRequestInterceptor;
...
public class HttpGlobalConfig {
    ...
    public HttpGlobalConfig proxy(Proxy proxy) {
        GoHttp.getOkHttpBuilder().proxy(checkNotNull(proxy, "proxy == null"));
        return this;
    }
    ...
    public HttpGlobalConfig connectTimeout(int timeout, TimeUnit unit) {
        if (timeout > -1) {
            GoHttp.getOkHttpBuilder().connectTimeout(timeout, unit);
        } else {
    ...
    public HttpGlobalConfig writeTimeout(int timeout, TimeUnit unit) {
        if (timeout > -1) {
            GoHttp.getOkHttpBuilder().writeTimeout(timeout, unit);
        } else {
    ...
    public HttpGlobalConfig readTimeout(int timeout, TimeUnit unit) {
        if (timeout > -1) {
            GoHttp.getOkHttpBuilder().readTimeout(timeout, unit);
        } else {
    ...
    public HttpGlobalConfig interceptor(Interceptor interceptor) {
        GoHttp.getOkHttpBuilder().addInterceptor(checkNotNull(interceptor, "interceptor == null"));
        return this;
    }
    ...
    public HttpGlobalConfig networkInterceptor(Interceptor interceptor) {
        GoHttp.getOkHttpBuilder().addNetworkInterceptor(checkNotNull(interceptor, "interceptor == null"));
        return this;
    }
    ...
    public HttpGlobalConfig cacheOffline(Cache httpCache) {
        networkInterceptor(new OfflineCacheInterceptor(GoHttp.getContext()));
        interceptor(new OfflineCacheInterceptor(GoHttp.getContext()));
        this.httpCache = httpCache;
        return this;
    }
    ...
    public HttpGlobalConfig cacheOffline(Cache httpCache, final int cacheControlValue) {
        networkInterceptor(new OfflineCacheInterceptor(GoHttp.getContext(), cacheControlValue));
        interceptor(new OfflineCacheInterceptor(GoHttp.getContext(), cacheControlValue));
        this.httpCache = httpCache;
        return this;
    }
    ...
    public int getRetryDelayMillis() {
        if (retryDelayMillis <= 0) {
            retryDelayMillis = GoConfig.DEFAULT_RETRY_DELAY_MILLIS;
        }
    ...
    public int getRetryCount() {
        if (retryCount <= 0) {
            retryCount = GoConfig.DEFAULT_RETRY_COUNT;
        }
        return retryCount;
    }
